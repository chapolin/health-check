...
var w = 450, h = 450, r = 5, padding = 2;

var buildHealthCheck = function() {
  var toggle = 0;
  var linkedByIndex = {};
  var optArray = [];
  var olor = d3.scale.category20();
...
var checkServices = function(data) {
...
var hasAttributeInObject = function(object, attribute, value) {
  return object!= null && object.hasOwnProperty(attribute) &&
    object.actived === value;
};

var healthCheckPing = function() {
  var userData = localStorage.getObj("health-check-user-data");

  if(
    // First Step: Verifying if plugin is actived
    hasAttributeInObject(userData, "actived", true) &&

    // Second Step: Checking connections
    checkConnections()
  ) {
    //console.log("Ativo");
    healthCheckGetData(function(a, b) {
      //console.log(a, b);
    });
  }
};
...
var healthCheckGetData = function(callback) {
  $.ajax({
    type: "HEAD",
    async: true,
    timeout: 100,
    url: "http://napsao-qa-nix-fend-ng-app-1.qa.vmcommerce.intra:8080/services/healthcheck",
    success: function() {
      $.ajax({
        type: "GET",
        async: true,
